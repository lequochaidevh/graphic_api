...
void TestUniform::OnUpdate(float deltaTime) {
    // Ideally we would actually specify a "rate" instead of a step, and then we
    // would normalize the points stepped based on the input deltaTime, i.e. if
    // rate = 100 points / sec, or 100 points / 1000 ms, or 0.1 points / ms,
    // then we pass in delta time in ms and the points we should step are 0.1 *
    // deltaTime * direction.
    if (m_Oscillate) {
        // increment red
        float *red = m_ObjectColor;
        if (*red < 0.0f) m_Direction = 1;
        if (*red > 1.0f) m_Direction = -1;
...
void TestUniform::OnRender() {
    ...
    m_shader.Bind();
    m_shader.SetUniform4f("u_Color", m_ObjectColor[0], m_ObjectColor[1],
                          m_ObjectColor[2], m_ObjectColor[3]);
